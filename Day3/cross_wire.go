...
func (cw *CrossWire) FewestIntersectionStepsTotal() int {
	var fewestSteps int

	intersections := cw.Intersections()

	for index, point := range intersections {
		resultA, _ := cw.WireA.NumberOfSteps(point)
		resultB, _ := cw.WireB.NumberOfSteps(point)
		result := resultA + resultB

		if fewestSteps > result || index == 0 {
			fewestSteps = result
		}
	}
